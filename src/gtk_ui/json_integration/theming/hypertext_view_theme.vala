public class Fossil.GtkUi.JsonIntegration.Theming.HypertextViewTheme {
	
	public static Json.Object hyper_text_view_theme_to_json(Fossil.GtkUi.Theming.HypertextViewTheme theme){
		...
		theme.foreach_text_tag_theme((name,tag_theme) => {
			prefix_object.set_object_member(name, Fossil.GtkUi.JsonIntegration.Theming.TextTagTheme.text_tag_theme_to_json(tag_theme));
		});
			...
			if (tag_theme_node.get_node_type() == OBJECT){
				...
				tag_theme_object.foreach_member((_, name, node) => {
					if (node.get_node_type() == OBJECT) {
						theme.set_text_tag_theme(name, Fossil.GtkUi.JsonIntegration.Theming.TextTagTheme.text_tag_theme_from_json(node.get_object()));
					}
				});
			}
		}
