public class Fossil.GtkUi.LegacyUtil.GtkScrollExport {
	
	public static bool import(Gtk.ScrolledWindow sw, string data){
		var kv = new Fossil.Util.Kv();
		kv.import(data);
		if (kv.get_value("type") != "fossil.gtk_scroll_export.0"){
			return false;
		}
		string? val = kv.get_value("vscroll");
		if (val != null){
			uint64 vscroll;
			if (Fossil.Util.Intparser.try_parse_unsigned(val,out vscroll)){
				Timeout.add(100,() => {
		...
		if (val != null){
			uint64 hscroll;
			if (Fossil.Util.Intparser.try_parse_unsigned(val,out hscroll)){
				Timeout.add(100,() => {
	...
	public static string export(Gtk.ScrolledWindow sw){
		var kv = new Fossil.Util.Kv();
		kv.set_value("type","fossil.gtk_scroll_export.0");
