public class Dragonstone.GtkUi.Widget.InlineSearch : Gtk.Bin {
	...
	private bool handle_button_press(Gdk.EventButton event){
		if (event.type == BUTTON_PRESS){
			if (event.button == 3 && uri_template.has_suffix("/postfile%09{search}")) { //right click
				var popover = new Dragonstone.GtkUi.Widget.InlineSearchPostB64FilePopoverEasterEgg(this,uri_template.substring(0,uri_template.length-11)+"b64");
