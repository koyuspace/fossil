public class Dragonstone.Startup.Sessions.Gtk {
	public static void setup_views(Dragonstone.SuperRegistry super_registry){
		...
		if (view_registry != null){
			...
			view_registry.add_rule(new Dragonstone.GtkUi.ViewRegistryRule("interactive/tls_session","dragonstone.tls_session"));
			view_registry.add_rule(new Dragonstone.GtkUi.ViewRegistryRule("error/uri/unknownScheme","dragonstone.no_session_panel").prefix("session://"));
		}
	}
