public class Dragonstone.Util.SettingsFetcher {
	public static Dragonstone.Interface.Settings.Rom? fetch_settings_rom_from_superregistry(string id,  Dragonstone.SuperRegistry super_registry, out Dragonstone.Asm.Scriptreturn? error = null){
		...
		Dragonstone.Interface.Settings.Provider? provider = (Dragonstone.Interface.Settings.Provider) super_registry.retrieve(provider_name);
		if (provider == null){
		...
		Dragonstone.Interface.Settings.Rom? rom = provider.get_object(rom_name);
		if (rom == null){
