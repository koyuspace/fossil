public class Dragonstone.View.ConnectionRefused : Gtk.Bin, Dragonstone.IView {
	...
	construct {
		...
		var label = new Gtk.Label("Connection Refused"); //TOTRANSLATE
		var sublabel = new Gtk.Label("so rude ..."); //TOTRANSLATE
		var icon = new Gtk.Image.from_icon_name("action-unavailable-symbolic",Gtk.IconSize.DIALOG);
		icon.icon_size=6;
	...
	public bool displayResource(Dragonstone.Resource resource,Dragonstone.Tab tab){
		if (!(resource.resourcetype == Dragonstone.ResourceType.ERROR_CONNECTION_REFUSED)) {return false;}
	...
	public bool canHandleCurrentResource(){
		if (resource == null){
			...
			return resource.resourcetype == Dragonstone.ResourceType.ERROR_CONNECTION_REFUSED;
		}
	}
